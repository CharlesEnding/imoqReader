...
# proc `$`(t: ModelKind):   string = result = if t.KnownModelKind in KNOWN_MODEL_KINDS: $KnownModelKind(t) else: "UnknownModelKind" & $t.int
...
proc readTexture*(s: Stream): Texture =
  discard s.readData(result.addr, sizeof(Texture)-sizeof(result.textureIndices))
  result.textureIndices = s.readSeq(result.textureDataSize.int * 4, uint8)
